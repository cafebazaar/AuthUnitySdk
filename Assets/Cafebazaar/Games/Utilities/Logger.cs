namespace CafeBazaar.Games.OurUtils
{
    ...
    public class Logger
    {
        ...
        private static string ToLogMessage(string prefix, string logType, string msg)
        {
            ...
            {
                Debug.LogWarning("*** [CafeBazaar Games Plugin " + AuthAndStorageBridge.Instance.GetVersion() + "] ERROR: Failed to format DateTime.Now");
                timeString = string.Empty;
            }

            return string.Format("{0} [CafeBazaar Games Plugin " + AuthAndStorageBridge.Instance.GetVersion() + "] {1} {2}: {3}",
